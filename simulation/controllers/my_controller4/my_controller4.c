...
int main(int argc, char **argv) {
  ...
  for (m = 0; m < 4; ++m) {
    ...
    wb_motor_set_velocity(motors[m], 1);
  }
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    const double rear_right_motor_input = k_vertical_thrust + vertical_input + roll_input - pitch_input - yaw_input;
      wb_motor_set_velocity(propeller1, front_left_motor_input);
    wb_motor_set_velocity(propeller2, -front_right_motor_input);
    wb_motor_set_velocity(propeller3, -rear_left_motor_input);
    wb_motor_set_velocity(propeller4, rear_right_motor_input);
  };
